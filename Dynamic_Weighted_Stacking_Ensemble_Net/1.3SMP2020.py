...
model = ErnieForSequenceClassification.from_pretrained(model_name, num_classes=6)
...
def train_and_evaluate(model, tokenizer, train_data_loader, dev_data_loader, epochs, learning_rate, warmup_proportion, weight_decay):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if accu > pre_accu:
            ...
            save_param_path = os.path.join(save_dir,
                                           f'model_state_E{epoch}_accu_{accu:.4f}_F1_{f1:.4f}_loss_{loss:.4f}_recall_{recall:.4f}.pdparams')  # 保存模型参数
    ...
    return pre_accu, best_precision, best_f1, best_recall, best_loss

# 训练和评估
print(f'Training and evaluating {model_name}...')
acc, precision, f1, recall, loss = train_and_evaluate(model, tokenizer, train_data_loader, dev_data_loader, epochs, learning_rate, warmup_proportion, weight_decay)

print('Results:')
print(f'{model_name}: Accuracy: {acc:.4f}, Precision: {precision:.4f}, F1: {f1:.4f}, Recall: {recall:.4f}, Loss: {loss:.4f}')
