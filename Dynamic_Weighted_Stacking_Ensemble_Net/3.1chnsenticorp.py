...
def train_and_evaluate(model, tokenizer, train_data_loader, dev_data_loader, epochs, learning_rate, warmup_proportion, weight_decay, model_name, fold_idx):
    ...
    global_step = 0
    total_train_time = 0
    fold_start_time = time.time()  # 在fold的开始定义
    for epoch in range(1, epochs + 1):
        ...
        epoch_train_time = time.time() - fold_start_time
...
# 加载训练集
dataset = ChnSentiCorpData('train.tsv')

# 定义模型和对应的tokenizer
model_nezha = CustomNeZhaForSequenceClassification.from_pretrained('nezha-base-chinese', num_classes=2)
...
for fold_idx, (train_idx, dev_idx) in enumerate(kf.split(dataset)):
    ...
    # 创建fold对应的数据集
    train_subset = paddle.io.Subset(dataset, train_idx)
    dev_subset = paddle.io.Subset(dataset, dev_idx)
    ...
    meta_ce_loss_value = float(meta_ce_loss(final_probs_tensor, labels_tensor))  
    meta_kl_loss_value = float(meta_kl_loss(F.log_softmax(final_probs_tensor, axis=-1), final_probs_tensor))  
