...
def evaluate(model, criterion, metric, data_loader):
    ...
    for batch in data_loader:
        ...
        loss = criterion(logits, labels, probs)  # 使用MixedLoss计算损失
...
def train_and_evaluate(model, tokenizer, train_data_loader, dev_data_loader, epochs, learning_rate, warmup_proportion,
                       weight_decay, model_name, fold_idx):
    ...
    # 使用MixedLoss而不是CrossEntropyLoss
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            global_step += 1
            if global_step % 10 == 0:
                print("global step %d, epoch: %d, batch: %d, loss: %.5f, acc: %.5f, f1: %.5f" % (
        ...
        print(
            f'Epoch {epoch}, Accuracy: {accu:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}, Loss: {loss:.4f}')
        if accu > pre_accu:
            save_param_path = os.path.join(save_dir,
                                           f'{model_name}_fold{fold_idx}_epoch{epoch}_accu_{accu:.4f}_F1_{f1:.4f}_loss_{loss:.4f}_recall_{recall:.4f}.pdparams')
    ...
    print(
        f'Best model from Epoch {best_epoch}, Accuracy: {pre_accu:.4f}, Precision: {best_precision:.4f}, F1: {best_f1:.4f}, Recall: {best_recall:.4f}, Loss: {best_loss:.4f}')
...
model_xlnet = XLNetForSequenceClassification.from_pretrained('xlnet-large-cased', num_classes=6,
...
for fold_idx, (train_idx, dev_idx) in enumerate(kf.split(train_ds)):
    ...
    train_data_loader_nezha = create_dataloader(train_subset, mode='train', batch_size=batch_size,
                                                batchify_fn=batchify_fn_nezha, trans_fn=trans_func_nezha)
    dev_data_loader_nezha = create_dataloader(dev_subset, mode='dev', batch_size=batch_size,
                                              batchify_fn=batchify_fn_nezha, trans_fn=trans_func_nezha)

    print('Training and evaluating NeZha...')
    acc_nezha, precision_nezha, f1_nezha, recall_nezha, loss_nezha, probs_nezha, labels_nezha = train_and_evaluate(
        model_nezha, tokenizer_nezha, train_data_loader_nezha, dev_data_loader_nezha, epochs, learning_rate,
        warmup_proportion, weight_decay, "NeZha", fold_idx)

    # 训练和评估 XLNet 模型
    trans_func_xlnet, batchify_fn_xlnet = get_trans_func_and_batchify_fn(tokenizer_xlnet)
    train_data_loader_xlnet = create_dataloader(train_subset, mode='train', batch_size=batch_size,
                                                batchify_fn=batchify_fn_xlnet, trans_fn=trans_func_xlnet)
    dev_data_loader_xlnet = create_dataloader(dev_subset, mode='dev', batch_size=batchify_fn_xlnet,
                                              trans_fn=trans_func_xlnet)

    print('Training and evaluating XLNet...')
    acc_xlnet, precision_xlnet, f1_xlnet, recall_xlnet, loss_xlnet, probs_xlnet, _ = train_and_evaluate(model_xlnet,
                                                                                                        tokenizer_xlnet,
                                                                                                        train_data_loader_xlnet,
                                                                                                        dev_data_loader_xlnet,
                                                                                                        epochs,
                                                                                                        learning_rate,
                                                                                                        warmup_proportion,
                                                                                                        weight_decay,
    ...
    train_data_loader_ernie = create_dataloader(train_subset, mode='train', batch_size=batch_size,
                                                batchify_fn=batchify_fn_ernie, trans_fn=trans_func_ernie)
    dev_data_loader_ernie = create_dataloader(dev_subset, mode='dev', batchify_fn=batchify_fn_ernie,
                                              trans_fn=trans_func_ernie)

    print('Training and evaluating ERNIE...')
    acc_ernie, precision_ernie, f1_ernie, recall_ernie, loss_ernie, probs_ernie, _ = train_and_evaluate(model_ernie,
                                                                                                        tokenizer_ernie,
                                                                                                        train_data_loader_ernie,
                                                                                                        dev_data_loader_ernie,
                                                                                                        epochs,
                                                                                                        learning_rate,
                                                                                                        warmup_proportion,
                                                                                                        weight_decay,
    ...
    def stacking_loss_function(weight_nezha, weight_xlnet, weight_ernie):
        final_probs = (weight_nezha * probs_nezha + weight_xlnet * probs_xlnet + weight_ernie * probs_ernie) / (
                    weight_nezha + weight_xlnet + weight_ernie)
    ...
    # 应用最优权重进行加权平均
    final_probs_weighted = (
                                       best_weight_nezha * probs_nezha + best_weight_xlnet * probs_xlnet + best_weight_ernie * probs_ernie) / (
                                       best_weight_nezha + best_weight_xlnet + best_weight_ernie)
    ...
    print(
        f'  Current ensemble model - Accuracy: {stacking_acc:.4f}, Precision: {stacking_precision:.4f}, Recall: {stacking_recall:.4f}, F1: {stacking_f1:.4f}, Loss: {stacking_loss:.4f}')
    print(
        f'  Best ensemble model so far - Accuracy: {best_meta_results["best_meta_acc"]:.4f}, Precision: {best_meta_results["best_meta_precision"]:.4f}, Recall: {best_meta_results["best_meta_recall"]:.4f}, F1: {best_meta_results["best_meta_f1"]:.4f}, Loss: {best_meta_results["best_meta_loss"]:.4f}')
...
print(
    f'Best ensemble model - Accuracy: {best_meta_results["best_meta_acc"]:.4f}, Precision: {best_meta_results["best_meta_precision"]:.4f}, Recall: {best_meta_results["best_meta_recall"]:.4f}, F1: {best_meta_results["best_meta_f1"]:.4f}, Loss: {best_meta_results["best_meta_loss"]:.4f}')
