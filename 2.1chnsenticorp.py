...
dev_ds = ChnSentiCorpData('dev.tsv')
test_ds = ChnSentiCorpData('test.tsv')

# 定义模型和对应的tokenizer
model_nezha = CustomNeZhaForSequenceClassification.from_pretrained('nezha-large-wwm-chinese', num_classes=2)
...
# 计算融合模型的损失值
meta_loss_ce = nn.CrossEntropyLoss()
meta_loss_kl = nn.KLDivLoss(reduction='batchmean')
final_probs_tensor = paddle.to_tensor(final_probs)
labels_tensor = paddle.to_tensor(labels_nezha)

meta_ce_loss = meta_loss_ce(final_probs_tensor, labels_tensor)
meta_kl_loss = meta_loss_kl(F.log_softmax(final_probs_tensor, axis=-1), final_probs_tensor)
meta_loss = 0.5 * meta_ce_loss + 0.5 * meta_kl_loss

print(f'Optimized Ensemble Model - Accuracy: {meta_acc:.4f}, Precision: {meta_precision:.4f}, Recall: {meta_recall:.4f}, F1: {meta_f1:.4f}, Loss: {meta_loss.item():.4f}')
